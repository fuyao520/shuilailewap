...
var loadingProcess = {
    ...
    progressPicHandler: function (a, b, c) {
        ...
        if (a == '100%') {
            picShow.$("Display").src = picShow.Img.src;
        }
    ...
    load: function (a) {
        picShow.setTit();
        loadingProcess.completePicHandler(a);
        return;
        (picShow.isIE ?
        //        loadingProcess.thisMovie("picMe")     
(loadingProcess.completePicHandler(a))
        //(picShow.showPic(picShow.Img.src))
         : loadingProcess.thisMovie("picMeFF").picLoader(a));
    },
    loadPic: function () {
        var a = new Image;
        a.src = this.url;
...
picShow = {
    ...
    lastGoto: function (a) {
        if (a == "open") {
            ...
            this.$("rePlay").href = "javascript:void(0);";
    ...
    Loader: function () {
        for (var a = 0; a < this.Count; a++) {
            this.Src[a] = photoJson[a].smallpic
        }
        if (picShow.isIE) {
            picShow.$("Display").style.opacity = 1;

            picShow.$("Display").style.filter = "alpha(opacity:100%)";
        } 
        else
        ...
        function () {
            picShow.$("preArrow").style.cursor = "url(/static/lib/gallery/arr_left.cur),auto"
        });
        addEventHandler(picShow.$("nextArrow"), "mousemove",
        function () {
            picShow.$("nextArrow").style.cursor = "url(/static/lib/gallery/arr_right.cur),auto"
        });
...
/*  |xGv00|0d5fd2ecb022db5988389bbf77d82197 */
