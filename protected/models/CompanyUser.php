...
class CompanyUser extends CActiveRecord{
	...
	function get_company_uphone_info($uphone=''){
		global $dbm;
		$sql="select  * from company_user_list where uphone='$uphone'  ";
	...
	//检查email是否存在,0 表示不存在，其他 返回  用户的数据
	function get_company_uname_info($uname=''){
		global $dbm;
		$sql="select  * from company_user_list where uname='$uname'  ";
		$rsu['list']=Yii::app()->db->createCommand($sql)->queryAll();
		if(count($rsu['list'])==0){
			return 0;
		}
	...
	//检查email是否存在,0 表示不存在，其他 返回  用户的数据
	function get_company_email_info($email=''){
		$sql="select * from company_user_list where uemail='$email'  ";
		$rsu['list']=Yii::app()->db->createCommand($sql)->queryAll();
		if(count($rsu['list'])==0){
			return 0;
		}
	...
	public function reg_new($uphone,$uname,$email,$upass,$group_id,$uqq){
		...
		if($this->get_company_uphone_info($uphone)>0){
			return -6;
		}
	    if($this->get_company_uname_info($uname)>0){
			return -2;
		}
		if($this->get_company_email_info($email)>0){
		    return -1;
		}
		$reg_time=time();
