...
function post_flag(){
	...
	$.get("<?php echo $this->createUrl('info/setFlag'); ?>?model_id=<?php echo $this->get('cate_id'); ?>&ids="+idarrs+"&flag="+flag_value,function(data){
...
function post_cate(){
	...
	$.get("<?php echo $this->createUrl('info/setInfoCate'); ?>?model_id=<?php echo $this->get('cate_id'); ?>&ids="+idarrs+"&set_cate_id="+cate_id,function(data){
		//alert(data);
		window.location=window.location.href;	
	});
	}catch(e){alert(e.message)}	
...
function del_recommend(table,info_id,recommend_id){
	$.get("<?php echo $this->createUrl('info/delRecommend'); ?>?model_id=<?php echo $this->get('cate_id'); ?>&table="+table+"&info_id="+info_id+"&recommend_id="+recommend_id,function(jsonstr){
		try{
		var json=eval("("+jsonstr+")");
		if(json.code<=0){
...
function post_set_special(){
	...
	$.get("<?php echo $this->createUrl('info/setSpecial'); ?>?model_id=<?php echo $this->get('cate_id'); ?>&ids="+idarrs+"&set_special_id="+special_id+"&small_cate="+small_cate,function(jsonstr){
		//alert(data);
		try{
		var json=eval("("+jsonstr+")");
		if(json.code<=0){
...
function del_special(model_id,info_id,special_id){
	$.get("<?php echo $this->createUrl('info/delSpecial'); ?>?model_id=<?php echo $this->get('cate_id'); ?>&model_id="+model_id+"&info_id="+info_id+"&special_id="+special_id,function(jsonstr){
		try{
		var json=eval("("+jsonstr+")");
		if(json.code<=0){
			alert(json.statewords)
...
<div class="main mhead">
    <div class="snav">内容中心 »<?php echo $page['model_info']['model_name']; ?>管理 </div>
    <div class="mt10">
    	<form>
    		<input type="hidden" name="select_mode" value="<?php echo $select_mode;?>">
    		<input type="hidden" name="model_id" value="<?php echo $this->get('model_id');?>">
    ...
    <div class="mt10 clearfix" <?php if($select_mode==1){?>style="display:none;"<?php }?>>
        <div class="l">
            ...
            <?php $this->check_u_menu(array('code'=>'<input type="button" class="but2" value="删除选中" onclick="set_some(\''.$this->createUrl('info/delete').'?model_id='.$this->get('model_id').'&ids=[@]\',\'确定删除吗？\');" />','auth_tag'=>'info_delete')); ?>
            <?php if(!isset($page['cate_info']['field_setting']['set_audit'])||$page['cate_info']['field_setting']['set_audit']==0){?>
			<?php $this->check_u_menu(array('code'=>'<input type="button" class="but2" value="已审核" onclick="set_some(\''.$this->createUrl('info/auditInfo').'?model_id='.$this->get('model_id').'&ids=[@]&audit=1\',\'确定设置为已审核吗？\');" />','auth_tag'=>'info_audit')); ?>
            <?php $this->check_u_menu(array('code'=>'<input type="button" class="but2" value="未审核" onclick="set_some(\''.$this->createUrl('info/auditInfo').'?model_id='.$this->get('model_id').'&ids=[@]&audit=0\',\'确定设置为未审核吗？\');" />','auth_tag'=>'info_audit')); ?>
            <?php }?>
            ...
            <?php $this->check_u_menu(array('code'=>'<input type="button" class="but2" value="添加'.$page['model_info']['model_name'].'" onclick="location=\''.$this->createUrl('infoModelList/update').'?model_id='.$this->get('model_id').'\'" />','auth_tag'=>'info_update')); ?>
...
<form action="<?php echo $this->createUrl('info/saveOrder');?>?model_id=<?php echo $this->get('cate_id'); ?>" name="form_order" method="post" >
<table class="tb" >
    <tr>
        ...
        <?php echo helper::field_paixu(array('url'=>''.$this->createUrl('info/index').'?model_id='.$this->get('model_id').'&p='.$_GET['p'].'','field_cn'=>'ID','field'=>'info_id')); ?>
        </th>
        <?php if(!isset($page['cate_info']['field_setting']['info_title'])||$page['cate_info']['field_setting']['info_title']==0){?>
        <th  class="alignleft"><?php echo helper::field_paixu(array('url'=>''.$this->createUrl('info/index').'?model_id='.$this->get('model_id').'&p='.$_GET['p'].'','field_cn'=>show_field_txt($page['mymodelfs'],'info_title'),'field'=>'info_title')); ?></th>
        <?php }?>
        <th><?php echo helper::field_paixu(array('url'=>''.$this->createUrl('info/index').'?model_id='.$this->get('model_id').'&p='.$_GET['p'].'','field_cn'=>'栏目','field'=>'last_cate_id')); ?></th>
        <?php if($select_mode!=1){?>
        <?php foreach($page['model_fields'] as $f){?>
        <th title="<?php echo $f['field_txt']; ?>"  <?php if(isset($page['cate_info']['field_setting'][$f['field_name']])&&$page['cate_info']['field_setting'][$f['field_name']]==1){echo 'style="display:none;"';} ?>><div style=" height:20px;  line-height:20px;max-width:100px;overflow:hidden; text-align:center;"><?php echo helper::field_paixu(array('url'=>''.$this->createUrl('info/index').'?model_id='.$this->get('model_id').'&p='.$_GET['p'].'','field_cn'=>$f['field_txt'],'field'=>$f['field_name'])); ?></div></th>
       <?php } ?>
       <?php }?> 
      	<?php if($select_mode==1){?>
        <th width="40">图片</th>
   ...
   ?>
    <tr>   
		...
		<?php if($r['audit']==0) echo '<a href="'.$this->createUrl('info/index').'?model_id='.$this->get('model_id').'&p='.$_GET['p'].'&audit=0" title="点击查看所有未审核"><font color=red>[未审核]</font></a>'; ?>
		...
		<a title="<?php echo $r['info_title']; ?>" href="<?php echo $this->createUrl('infoModelList/update');?>?model_id=<?php echo $this->get('model_id');  ?>&p=<?php echo $_GET['p'];  ?>&id=<?php echo $r['info_id']; ?>" <?php if($select_mode==1){?>onclick="return false;"<?php }?>><?php echo stripslashes($r['info_title']); ?></a>
        <?php }?>
        </div>
        <?php $recommends=Recommend::model()->get_info_recommends($page['model_info']['model_table_name'],$r['info_id']); ?>
        <?php $specials=Special::model()->get_info_specials($page['model_info']['model_id'],$r['info_id']); ?>
        ...
        <div style=" line-height:20px; overflow:hidden;" class="infotitlerecommend_box">
            ...
            <a onclick="return dialog_frame(this);" title="点击查看此推荐位所有信息" href="<?php echo $this->createUrl('recommend/update');?>?id=<?php echo $r2['recommend_id']; ?>" ><?php echo $r2['recommend_name']; ?></a><i class="delx" title="取消" onclick="del_recommend('<?php echo $page['model_info']['model_table_name']; ?>',<?php echo $r['info_id']; ?>,<?php echo $r2['recommend_id']; ?>);">X</i>
            <?php }?>
            <?php foreach($specials as $r2){  ?>
            <a onclick="return dialog_frame(this);" title="点击查看此专题的所有信息" href="<?php echo $this->createUrl('specialInfoRelation/Index');?>?special_id=<?php echo $r2['special_id']; ?>" ><?php echo $r2['special_name']; ?></a><i class="delx" title="取消" onclick="del_special('<?php echo $page['model_info']['model_id']; ?>',<?php echo $r['info_id']; ?>,<?php echo $r2['special_id']; ?>);">X</i>
            <?php }?>
        </div>
        <?php }?>
        <td><a onclick="return dialog_frame(this)" href="<?php echo $this->createUrl('info/index');?>?cate_id=<?php echo $r['last_cate_id'];?>"><?php echo $r['cname'];?></a>
        </td>
        <?php if($select_mode!=1){?>
        <?php foreach($page['model_fields'] as $f){?>
       ...
       <?php if($select_mode==1){?>
        ...
        <?php if(!$select_mode){?>
			...
			<?php }?>
	        <?php $this->check_u_menu(array('code'=>'<a href="'.$this->createUrl('infoModelList/update').'?model_id='.$page['model_info']['model_id'].'&p='.$_GET['p'].'&id='.$r['info_id'].'">修改</a>','auth_tag'=>'info_edit')); ?>
	        <?php if(!isset($page['cate_info']['field_setting']['info_comment'])||$page['cate_info']['field_setting']['info_comment']==0){?>
	          <?php $this->check_u_menu(array('code'=>'<a href="'.$this->createUrl('Comment/index').'?search_type=fromid&search_txt='.$page['model_info']['model_table_name'].'-'.$r['info_id'].'" onclick="return dialog_frame(this);" >评论<span class=red>('.$r['comments_total'].')</span></a>','auth_tag'=>'comment_show')); ?>
	        <?php }?>
        
        <?php }else{?>
        <a href="javascript:void(0);" onclick="select_mode(<?php echo $r['info_id'];?>,'<?php echo $r['info_title']; ?>',<?php echo $page['model_info']['model_id'];?>,'<?php echo $page['model_info']['model_name'];?>',this);">选中</a>
        <?php }?>
           </td>
...
$(".info_img").click(function(){
	...
	var table='<?php echo $page['model_info']['model_table_name'];?>';	
