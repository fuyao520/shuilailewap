...
function preview(){
	...
	var cate_id=$("#last_cate_id").val();
	if(!cate_id){
		alert('请选择分类再进行预览');
		return false;
	}
	var newaction="/details/index?cate_id="+cate_id+"&info_id=88888888&preview=1";
...
function get_pinyin(){
	...
	$.get("<?php echo $this->createUrl('infoCategory/getPinyin'); ?>?str="+encodeURI($("#info_title").val()),function(jsondata){
	    try{
...
function del_resource(resource_id){
	try{
	$.get("<?php echo $this->createUrl('infoModelList/delResource'); ?>?model_id=<?php echo $page['model_info']['model_id']; ?>&resource_id="+resource_id,function(data){
		//alert(data);
...
<form id="formpost" method="post" action="<?php echo $this->createUrl('infoModelList/update'); ?>?model_id=<?php echo $page['model_info']['model_id']; ?>&p=<?php echo $_GET['p'];?>">
<input type="hidden" id="model_id" name="model_id" value="<?php echo $page['model_info']['model_id']; ?>" />
...
<div class="tab_table">
   <div class="title01"><?php echo $page['info']['info_id']?'修改'.$page['model_info']['model_name']:'添加'.$page['model_info']['model_name']; ?></div>
   <div class="btn_box">
...
<table class="tb3"  id="tab001">
    ...
    <?php $page['cate_info']['extern_content']=isset($page['cate_info']['extern_content'])?json_decode($page['cate_info']['extern_content'],1):array(); 
...
<table class="tb3">
    <tr>
       ...
       <?php }?>
        <input type="button" class="but" value="返回" onClick="window.location='<?php echo $this->createUrl('infoModelList/index');?>?model_id=<?php echo $page['model_info']['model_id']; ?>&p=<?php echo $_GET['p'];?>'" /></td>
    </tr>
</table>
</form>
</div>
