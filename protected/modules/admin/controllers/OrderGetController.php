<?php
class OrderGetController extends AdminController{

	public function actionIndex(){
		$this->auth_action('OrderGet_Index');
		//搜索
		$params['where']='';
		...
		$page['listdata']=Dtable::model(OrderGet::model()->tableName())->listdata($params);
	...
	public function actionUpdate(){ 
		...
		if(!isset($_POST['id'])){
			...
			if($id){
				$this->auth_action('OrderGet_edit');			
				$info=$this->toArr(OrderGet::model()->findByPk($id));
				if(!$info){
					$this->msg(array('state'=>0,'msgwords'=>'数据不存在'));
				}
				$page['info']=$info;
	...
	public function actionDelete(){
		$this->auth_action('OrderGet_del');
		$idstr=$this->get('ids');
		$ids=explode(',',$idstr);
		foreach($ids as $id){	
			$m=OrderGet::model()->findByPk($id);
			if(!$m) continue;
			$m->delete();
