...
class CompanyUserController extends AdminController{
	...
	public function actionIndex(){
		$this->auth_action('CompanyUser_Index');
		//搜索
		$params['where']='';
		...
		$params['select']="c.company_name,g.linkage_name as city_name,a.*";
		$params['order']="  order by a.uid      ";
		$params['pagesize']=Yii::app()->params['management']['pagesize'];
		$params['join']="left join linkage as g on g.linkage_id=a.city_id
						left join company as c on c.company_id=a.uid
				";
		$params['pagebar']=1;
		$params['smart_order']=1;
		$page['listdata']=Dtable::model(CompanyUser::model()->tableName())->listdata($params);
	...
	public function actionUpdate(){ 
		...
		if(!isset($_POST['id'])){
			...
			if($id){
				$this->auth_action('CompanyUser_edit');			
				$info=$this->toArr(CompanyUser::model()->findByPk($id));
				if(!$info){
					$this->msg(array('state'=>0,'msgwords'=>'数据不存在'));
				}
				$page['info']=$info;
				$page['company']=$this->toArr(Company::model()->findByPk($info['uid']));
				//print_r($page['recommend']);
			}else{
				$this->auth_action('CompanyUser_add');
				$page['company']=array();
			}
			...
			if($upass||!$id){
				...
				$field['upass']=CompanyUser::password_encryption($upass);
			}
			...
			if($dbresult===false){
				...
				//新增和修改之后的动作				
				$company_data=$this->post('company');
				Company::model()->save_company_profile($id,$company_data);
				$this->logs($logs);
				//成功跳转提示
				$this->msg($msgarr);
			}
	...
	public function actionChangeState(){
		$this->auth_action('CompanyUser_state');
		$idstr=$this->get('ids');
		$ids=explode(',',$idstr);
	...
	public function actionDelete(){
		$this->auth_action('CompanyUser_del');
		$idstr=$this->get('ids');
		$ids=explode(',',$idstr);
		foreach($ids as $id){	
			$m=CompanyUser::model()->findByPk($id);
